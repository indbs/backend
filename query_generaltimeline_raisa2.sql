Create temporary table temp1 SELECT  
	...
	concat('',timediff(min(TIME1),ifnull((select max(TIME1) from raisa_2 where program_number=mt1.program_number-1 and date(TIME1) >= '2019-01-01'),min(TIME1)))) as pause,
	ifnull(TIMESTAMPDIFF(SECOND,(Select max(TIME1) from raisa_2 where program_number=mt1.program_number and date(TIME1) >= '2019-01-01'), now()),0)  as currentWork  
FROM
	raisa_2 mt1 
WHERE 
	date(TIME1) >= '2019-01-01'
GROUP BY
