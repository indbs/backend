   	...
   	SELECT
		...
		max(time) as ВремяОкончания,
 		...
 		round(avg(waterflow)*hour(timediff(max(time),min(time))),2) as waterQuant,
   		...
   		ifnull(TIMESTAMPDIFF(SECOND,(Select max(time) from KILN_WORK_FINAL_LAST3 where program_number=mt1.program_number and year(date(time))=year(now())), now()),0)  as currentWork 
   	FROM 
...
USING 
	(PROGRAM_NUMBER)
WHERE
   	year(date(PROGRAMS_HEAT.inserted))=year(now()) and year(date(PROGRAMS_gas.inserted))=year(now())
GROUP BY
   	PROGRAM_NUMBER
ORDER BY
   	STARTUP_TIME;