		...
		max(time) as ВремяОкончания,
 		...
 		round(avg(waterflow)*hour(timediff(max(time),min(time))),2) as waterQuant,
   		...
   		concat('',timediff(max(time),min(time))) as duration,
   	   	concat('',timediff(min(time),ifnull((select max(time) from KILN_WORK_FINAL_LAST3 where program_number=mt1.program_number-1 and date(TIME) >= '2019-01-01'),min(time)))) as pause,
   		ifnull(TIMESTAMPDIFF(SECOND,(Select max(time) from KILN_WORK_FINAL_LAST3 where program_number=mt1.program_number and date(TIME) >= '2019-01-01'), now()),0)  as currentWork 
   	FROM 
...
USING 
	(PROGRAM_NUMBER)
WHERE
   	date(PROGRAMS_HEAT.inserted) >= '2019-01-01' and date(PROGRAMS_gas.inserted) >= '2019-01-01'
GROUP BY
   	PROGRAM_NUMBER
ORDER BY
   	STARTUP_TIME;